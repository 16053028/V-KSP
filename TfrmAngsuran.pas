...
begin
  with dbModule.dbQuery do
    begin
      ...
      ExecSQL;
      Open;
      kaliAngsuran:= StrToInt(dbModule.dbQuery.FieldByName('LAMA_ANGSURAN').Value);
  ...
  with dbModule.dbQuery do
    begin
      SQL.Clear;
      SQL.Add(Memo1.Text);
      SQL.Add('WHERE pinjaman_member.KODE_PINJAM = '+QuotedStr(in_CariPinjaman.Text));
      Open;
    end;
  if ((dbModule.dbQuery.RecordCount=0) or (dbModule.dbQuery.RecordCount>0)) then
    begin
      in_Ket.Text:=IntToStr(dbModule.dbQuery.RecordCount+1);
    end
  else if(kaliAngsuran>(dbModule.dbQuery.RecordCount+1)) then
    begin
  ...
  with dbModule.dbQuery do
    begin
      ...
      SQL.Add('VALUES ('+QuotedStr(in_KodeAngsuran.Text)+', '
                        ...
                        +QuotedStr(NIK)+', '
                        +QuotedStr(DateToStr(now))+', '
                        +QuotedStr('Bayar angsuran ke- '+in_Ket.Text)+');');
      ExecSQL;
      Close;
      conDB_MainTBL();
      Requery();
